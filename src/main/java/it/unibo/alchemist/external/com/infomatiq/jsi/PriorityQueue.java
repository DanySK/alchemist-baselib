...
public class PriorityQueue implements Serializable {
    ...
    public PriorityQueue(final boolean sortOrder, final int initialCapacity) {
     ...
     */
    private boolean sortsEarlierThan(final double p1, final double p2) {
    ...
    public void insert(final int value, final double priority) {
        values.add(value);
    ...
    private void promote(final int i, final int value, final double priority) {
        ...
        while (index > 0) {
            ...
            final double parentPriority = priorities.get(parentIndex);
    ...
    private void demote(final int i, final int value, final double priority) {
        int index = i;
        ...
        while (childIndex < values.size()) {
            ...
            if (childIndex + 1 < values.size()) {
                final double rightPriority = priorities.get(childIndex + 1);
    ...
    public int pop() {
        final int ret = values.get(0);

        // record the value/priority of the last entry
        final int lastIndex = values.size() - 1;
        final int tempValue = values.get(lastIndex);
        final double tempPriority = priorities.get(lastIndex);
    ...
    private void check() {
        ...
        for (int i = 0; i < values.size() / 2; i++) {
            final double currentPriority = priorities.get(i);

            final int leftIndex = (i * 2) + 1;
