 ...
 * 
 * The elements in the list are always ordered by priority. Methods exists to
 * remove elements with the highest and lowest priorities.
 * 
 * If more than one element has the highest priority, they will all be removed
 * by calling removeHighest. Ditto for the lowest priority.
 * 
 * The list has a preferred maximum size. If possible, entries with the lowest
 * priority will be removed to limit the maximum size. Note that entries with
 * the lowest priority will not be removed if this would leave fewer than the
 * preferred maximum number of entries.
 * 
 * This class is not optimised for large values of preferredMaximumSize. Values
